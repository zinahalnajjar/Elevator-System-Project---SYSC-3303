 ...
 * FloorSubsyste
 * 
 * floor subsystem thread that will send ------old code--------
 * 
...
/*
 * Floor subsystem sends the data read from the input file to the scheduler
 * subsystem receive commands to turn the floor button lamps off
 */
		...
		try {
			...
			while (scanner.hasNext()) {
				...
				try {
					...
					int error = Integer.valueOf(tokens[4]);
					FloorMovementData fD = new FloorMovementData(time, originFloor, destinationFloor, goingUp, error);
					floorInfo.add(fD);
				} catch (Exception e) {
